...
import { FiDollarSign, FiStar, FiAlertCircle, FiUsers, FiBox, FiTruck } from 'react-icons/fi';
...
const Dashboard = () => {
  ...
  const stats = [
    ...
    { label: 'Outstanding Dues', value: `$${Number(data.outstandingDues || 0).toLocaleString()}`, icon: FiAlertCircle, color: '#dc2626' },
  ];
        ...
        {stats.map((s, i) => (
          <div className="stat-card" key={i}>
            <div className="stat-info">
              ...
              {s.change && <span className="stat-change" style={{ color: s.color }}>{s.change}</span>}
            </div>
